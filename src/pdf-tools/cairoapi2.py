...
def get_num_of_pages(file_in):
    input1 = PdfFileReader(open(file_in, 'rb'))
    return input1.getNumPages()


def get_size(rectangle):
    x0, y0 = rectangle.lowerLeft
    x1, y1 = rectangle.upperRight
    return float(abs(x1 - x0)), float(abs(y1 - y0))
...
def extractText(file_in, file_out):
    with open(file_out, 'w') as document_out:
        document_in = PdfFileReader(open(file_in, 'rb'))
        for i in range(0, document_in.getNumPages()):
            print(document_in.getPage(i).extractText())
            document_out.write(document_in.getPage(i).extractText().decode())
...
def resize(file_in, file_out, width=1189, height=1682):
    document_in = PdfFileReader(open(file_in, 'rb'))
    document_out = PdfFileWriter()
    number_of_pages = document_in.getNumPages()
    width = float(width)
    height = float(height)
    horizontal = (width > height)
    current_page = document_in.getPage(0)
    print(get_size(current_page.mediaBox))
    for i in range(0, number_of_pages):
        current_page = document_in.getPage(i)
        '''
        widthi, heighti = get_size(current_page.mediaBox)
        ...
        if horizontal != horizontali:
            ...
            current_page.scale(scale, scale)
        else:
            sw = width / widthi
            sh = height / heighti
            if sw < sh:
            ...
            current_page.scale(scale, scale)
        '''
        current_page.scaleBy(2)
        document_out.addPage(current_page)
    document_out.write(file(file_out, 'wb'))
    '''
    for i in range(0, number_of_pages):
        current_page = document.getPage(i)
...
if __name__ == '__main__':
    resize('/home/lorenzo/Escritorio/pdfs/ejemplo_pdf_01.pdf',
          '/home/lorenzo/Escritorio/pdfs/ejemplo_pdf_01_redime.pdf')
    # encrypt('/home/lorenzo/Escritorio/pdfs/ejemplo_pdf_01_redime.pdf', '12345678')
